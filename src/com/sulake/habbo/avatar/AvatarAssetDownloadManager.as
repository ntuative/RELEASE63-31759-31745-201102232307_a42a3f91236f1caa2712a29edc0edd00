package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_960:int = 3;
      
      private var var_424:AssetLoaderStruct;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_424 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_958 = new Timer(this.const_1997,1);
         this.var_958.addEventListener(TimerEvent.TIMER_COMPLETE,this.method_10);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_424)
         {
            this.var_424.addEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_424.addEventListener(AssetLoaderEvent.const_45,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_424)
         {
            this.var_424.removeEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_424.removeEventListener(AssetLoaderEvent.const_45,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_960);
            if(this.var_1431.indexOf("?") > 0)
            {
               _loc2_ = this.var_1431 + "&retry=" + this.var_960;
            }
            else
            {
               _loc2_ = this.var_1431 + "?retry=" + this.var_960;
            ...
            this.var_424 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2590,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1170[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1844)
         {
            this.var_1171.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_484[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_484[_loc3_] = _loc6_;
            }
      ...
      {
         var _loc1_:* = null;
         while(this.var_767.length > 0 && this.var_768.length < this.const_1547)
         {
      ...
      private function method_10(param1:Event = null) : void
      {
         this.processPending();
      }
   }
