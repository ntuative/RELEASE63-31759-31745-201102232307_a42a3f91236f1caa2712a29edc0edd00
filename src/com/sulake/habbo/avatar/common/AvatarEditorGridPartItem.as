package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_772:IFigurePartSet;
      ...
      private var var_425:IAvatarRenderManager;
      
      private var var_764:Boolean;
      
      private var var_1862:BitmapData;
      
      private const const_1986:int = 13421772;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 == null)
         {
            this.var_650 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1863 = Math.max(this.var_1863,_loc6_.colorLayerIndex);
            }
         }
         this.var_425 = this.var_66.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_650 != null && !this.var_1861)
            {
               _loc3_ = this.var_650;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_772)
            {
               switch(this.var_772.clubLevel)
               {
                  case HabboClubLevelEnum.const_51:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_40:
                     _loc2_.style = HabboIconType.const_826;
                     break;
                  case HabboClubLevelEnum.const_41:
                     _loc2_.style = HabboIconType.const_665;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this._window.color = this.const_1986;
         }
         else
         {
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_425.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_425.isFigureReady(_loc1_))
         {
            this.var_425.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_327[_loc6_] + "_" + FigureData.const_128;
               _loc4_ = this.var_425.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
         ...
         if(this.var_629 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1862)
               {
                  ...
                  this.var_1862 = _loc9_.content as BitmapData;
               }
