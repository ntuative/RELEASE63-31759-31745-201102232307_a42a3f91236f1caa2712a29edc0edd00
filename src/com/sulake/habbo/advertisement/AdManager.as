package com.sulake.habbo.advertisement
{
   ...
   public class AdManager extends Component implements IAdManager
   {
      ...
      private var var_1176:BitmapData = null;
      
      private var var_1177:BitmapData = null;
      ...
      {
         this.var_294 = param2 as IHabboCommunicationManager;
         if(this.var_294 == null)
         {
            return;
         }
         this._connection = this.var_294.getHabboMainConnection(this.onConnectionReady);
      ...
      private function onConfigurationReady(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         this.var_2602 = this._config.getKey("interstitial.enabled","RoomWidgetCreditFurniUpdateEvent") == "HabboCatalogCom_productViewWidget";
         this.var_2603 = int(this._config.getKey("interstitial.interval","30000"));
         this.var_2600 = int(this._config.getKey("interstitial.show.time","5000"));
         this.var_1856 = int(this._config.getKey("interstitial.max.displays.flash","-1"));
      ...
      public function showInterstitial() : Boolean
      {
         ...
         if(this.var_226 != null && this.var_226.running)
         {
            this.var_226.reset();
            return false;
         }
         ...
         if(this.var_1856 > 0 && this.var_1857 >= this.var_1856)
         {
            return false;
         }
         var _loc1_:int = getTimer() - this.var_1178;
         if(_loc1_ > this.var_2603)
         {
            ++this.var_1857;
      ...
      private function onInterstitial(param1:InterstitialMessageEvent) : void
      {
         ...
         this.var_1437 = _loc2_.imageUrl;
         this.var_2601 = _loc2_.clickUrl;
         this.var_1178 = getTimer();
         if(this.var_1437 != "")
         {
            _loc3_ = this.var_1437;
            ...
            {
               this.var_226 = new Timer(this.var_2600,1);
               this.var_226.addEventListener(TimerEvent.TIMER_COMPLETE,this.interstitialTimerComplete);
            }
            if(this.var_226.running)
            {
               this.var_226.reset();
            }
            this.var_226.start();
            _loc4_ = new URLRequest(this.var_1437);
      ...
      private function startInterstitial(param1:Bitmap) : void
      {
         ...
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_530,0,0,param1.bitmapData,this.var_2601));
            this.var_1178 = getTimer();
         }
         else
         {
      ...
      private function onRoomAd(param1:RoomAdMessageEvent) : void
      {
         ...
         this.var_1438 = _loc2_.imageUrl;
         this.var_1855 = _loc2_.clickUrl;
         if(this.var_1438 != "" && this.var_1855 != "")
         {
            _loc3_ = this.var_1438;
            if(assets.hasAsset(_loc3_))
            {
               assets.removeAsset(assets.getAssetByName(_loc3_));
            }
            _loc4_ = new URLRequest(this.var_1438);
            _loc5_ = assets.loadAssetFromFile(_loc3_,_loc4_,"image/jpeg");
      ...
      private function roomAdImageReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(this.isValidAdImage(_loc3_))
         {
            ...
            events.dispatchEvent(new AdEvent(AdEvent.const_427,0,0,_loc4_,this.var_1855,this.var_1176,this.var_1177));
         }
      }
      ...
      private function adWarningLeftReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc3_ != null)
         {
            this.var_1176 = this.emulateBackgroundTransparency(_loc3_.bitmapData);
         }
      }
      ...
      private function adWarningRightReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc3_ != null)
         {
            this.var_1177 = this.emulateBackgroundTransparency(_loc3_.bitmapData);
         }
      }
