package com.sulake.habbo.localization
{
   ...
   public class HabboLocalizationManager extends CoreLocalizationManager implements IHabboLocalizationManager
   {
      ...
      private var var_1796:Dictionary;
      ...
      {
         this.var_1796 = new Dictionary();
         this.var_2475 = ["I","II","III","IV","V","VI","VII","VIII","IX","X","XI","XII","XIII","XIV","XV","XVI","XVII","XVIII","XIX","XX"];
         super(param1,param2,param3);
         this.var_1795 = (param2 & 0) > 0;
         if(!this.var_1795)
         {
            lock();
         }
      ...
      private function onHabboConfigurationInit(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         if(param2 != null)
         {
            ...
            if(!this.var_1795)
            {
               if("RoomWidgetCreditFurniUpdateEvent" == this.var_173.getKey("use.default.localizations","RoomWidgetCreditFurniUpdateEvent"))
               {
                  _loc5_ = this.var_173.getKey("external.texts.txt","external_texts.txt");
                  events.addEventListener(LocalizationEvent.const_157,this.onLocalizationLoaded);
                  super.loadLocalizationFromURL(_loc5_);
               }
            }
            _loc4_ = 1;
            while(this.var_173.keyExists("localization." + _loc4_))
            {
               _loc6_ = this.var_173.getKey("localization." + _loc4_,"");
               _loc7_ = this.var_173.getKey("localization." + _loc4_ + ".name","");
               _loc8_ = this.var_173.getKey("localization." + _loc4_ + ".url","");
      ...
      {
         var _loc2_:* = null;
         events.removeEventListener(LocalizationEvent.const_157,this.onLocalizationLoaded);
         if(this.var_173.keyExists("external.override.texts.txt"))
         {
            _loc2_ = this.var_173.getKey("external.override.texts.txt");
            events.addEventListener(LocalizationEvent.const_157,this.onLocalizationOverrideLoaded);
      ...
      {
         events.removeEventListener(LocalizationEvent.const_157,this.onLocalizationOverrideLoaded);
         unlock();
      }
      ...
      {
         this.var_1796[param1] = param2;
      }
      
      private function getBadgePointLimit(param1:String) : int
      {
         return this.var_1796[param1];
      }
      ...
      {
         return this.var_2475[Math.max(0,param1 - 1)];
      }
   }
}
