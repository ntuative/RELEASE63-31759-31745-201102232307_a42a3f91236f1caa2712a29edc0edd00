package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_107,this.onTextWindowEvent);
         ...
         this.var_509 = this.getPetTypeIndexFromProduct(_loc3_.localizationId);
         if(this.var_509 >= 8)
         {
            return false;
         }
         this.updateAvailableBreeds(_loc3_.localizationId);
         this.var_2597 = _loc3_.localizationId;
         this.var_364 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc5_ = _loc4_.avatarRenderManager.petDataManager.getPetData(this.var_509,this);
               if(_loc5_)
               {
                  this.var_364 = _loc5_.colors;
         ...
         this._offers.add(this.var_509,_loc3_);
         events.addEventListener(WidgetEvent.CWE_SELECT_PRODUCT,this.onSelectProduct);
         events.addEventListener(WidgetEvent.CWE_COLOUR_INDEX,this.onColourIndex);
         events.addEventListener(WidgetEvent.const_893,this.onApproveNameResult);
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_364)
         {
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(this.var_518 < 0 || this.var_518 > this.var_364.length)
         {
            this.var_518 = 0;
         }
         this.updateImage();
      ...
      {
         if(param1.productCode == this.var_2597)
         {
      ...
      private function parseSellableBreeds(param1:Array) : Array
      {
         ...
         {
            if(_loc3_.type == this.var_509 && _loc3_.sellable)
            {
      ...
      private function updateBreedSelections() : void
      {
         ...
         while(_loc3_ < this.var_199.length)
         {
            ...
            _loc2_.push(_loc1_.getKey(this.getRaceLocalizationKey(this.var_509,_loc5_),this.getRaceLocalizationKey(this.var_509,_loc5_)));
            _loc3_++;
         }
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_199 == null || this.var_779 >= this.var_199.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_518 >= 0 && this.var_518 < this.var_364.length)
         {
            _loc7_ = this.var_364[this.var_518] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_199[this.var_779] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_509,_loc3_,_loc2_,AvatarScaleType.const_55,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_276,AvatarAction.const_621);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_518 >= this.var_364.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_364[this.var_518] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_199[this.var_779] as int;
