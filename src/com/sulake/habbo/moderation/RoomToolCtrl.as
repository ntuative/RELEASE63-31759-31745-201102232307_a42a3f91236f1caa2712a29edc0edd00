package com.sulake.habbo.moderation
{
   ...
   public class RoomToolCtrl implements IDisposable, TrackedWindow
   {
      ...
      private var var_476:ITextFieldWindow;
      
      private var var_553:Boolean = true;
      
      private var var_914:ICheckBoxWindow;
      
      private var var_1383:ICheckBoxWindow;
      
      private var var_1382:ICheckBoxWindow;
      ...
      public function show() : void
      {
         ...
         this.var_33.connection.send(new GetModeratorRoomInfoMessageComposer(this.var_323));
         Logger.log("BEGINNING TO SHOW: " + this.var_323);
      }
      
      public function getType() : int
      {
         return WindowTracker.const_1564;
      }
      ...
      {
         this.var_58 = IItemListWindow(this._frame.findChildByName("list_cont"));
         var _loc1_:IWindow = this._frame.findChildByTag("close");
         _loc1_.procedure = this.onClose;
         this.var_476 = ITextFieldWindow(this._frame.findChildByName("message_input"));
         this.var_476.procedure = this.onInputClick;
         ...
         this.var_914 = ICheckBoxWindow(this._frame.findChildByName("kick_check"));
         this.var_1383 = ICheckBoxWindow(this._frame.findChildByName("lock_check"));
         this.var_1382 = ICheckBoxWindow(this._frame.findChildByName("changename_check"));
      ...
      {
         var _loc3_:IWindowContainer = IWindowContainer(this.var_58.getListItemByName(param2));
         ...
         if(!param1.exists)
         {
            this.disposeItemFromList(this.var_58,_loc3_);
            this.disposeItemFromList(this.var_58,this.var_58.getListItemByName("event_spacing"));
            return;
         }
      ...
      private function onChatlog(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_33.windowTracker.show(new ChatlogCtrl(new GetRoomChatlogMessageComposer(0,this._data.flatId),this.var_33,WindowTracker.const_699,this._data.flatId),this._frame,false,false,true);
      }
      ...
      {
         if(this.var_553 || this.var_476.text == "")
         {
            this.var_33.windowManager.alert("Alert","You must input a message to the user",0,this.onAlertClose);
            return;
         }
         var _loc2_:int = this.determineAction(param1,this.var_914.isSelected);
         this.var_33.connection.send(new ModeratorActionMessageComposer(ModeratorActionMessageComposer.const_1400,_loc2_,this.var_476.text,"",""));
         if(this.var_1383.isSelected || this.var_1382.isSelected || this.var_914.isSelected)
         {
            this.var_33.connection.send(new ModerateRoomMessageComposer(this._data.flatId,this.var_1383.isSelected,this.var_1382.isSelected,this.var_914.isSelected));
         }
         this.dispose();
      }
      ...
      private function determineAction(param1:Boolean, param2:Boolean) : int
      {
         if(param2)
         {
            return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1395);
         }
         return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1631);
      }
      ...
      private function onSelectTemplate(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc3_:String = this.var_33.initMsg.roomMessageTemplates[this.var_1087.selection];
         if(_loc3_ != null)
         {
            this.var_553 = false;
            this.var_476.text = _loc3_;
         }
      }
   }
}
