package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private var var_1696:IWindowContainer;
      ...
      private var var_1101:IContainerButtonWindow;
      
      private var var_1100:IContainerButtonWindow;
      ...
      private function prepareWindow() : void
      {
         ...
         this._window = IFrameWindow(this.var_77.getXmlWindow("ude_help"));
         this.var_1696 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1100 = this.var_1696.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1100)
         {
            this.var_1100.procedure = this.onButtonClick;
         }
         this.var_1101 = this.var_1696.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1101)
         {
            this.var_1101.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_247 >= this.var_79.length - 1)
         {
            this.var_1101.disable();
         }
         else
         {
            this.var_1101.enable();
         }
         if(this.var_247 <= 0)
         {
            this.var_1100.disable();
         }
         else
         {
            this.var_1100.enable();
         }
      }
      ...
      {
         ++this.var_247;
         this.var_247 = Math.min(this.var_247,this.var_79.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_247;
         this.var_247 = Math.max(this.var_247,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         this.var_79 = [];
         var _loc1_:int = 1;
         while(this.var_77.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            _loc2_ = this.var_77.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title");
            _loc3_ = this.var_77.localization.getLocalization("wiredfurni.help." + _loc1_ + ".img");
            _loc4_ = this.var_77.localization.getLocalization("wiredfurni.help." + _loc1_ + ".text");
            this.var_79.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         if(!this.var_79 || this.var_79.length == 0)
         {
            return;
         }
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_79[this.var_247][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_79[this.var_247][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_79[this.var_247][1]).value;
      ...
      {
         if(!param1 || !this.var_77 || !this.var_77.assets)
         {
            return;
         }
         var _loc2_:BitmapDataAsset = this.var_77.assets.getAssetByName(param1) as BitmapDataAsset;
         if(_loc2_ == null)
         {
      ...
      private function retrievePreviewAsset(param1:String) : void
      {
         ...
         var _loc2_:String = this.var_77.configuration.getKey("image.library.catalogue.url");
         var _loc3_:* = _loc2_ + param1 + ".gif";
         Logger.log("[ProductViewCatalogWidget] Retrieve Product Preview Asset: " + _loc3_);
         var _loc4_:URLRequest = new URLRequest(_loc3_);
         var _loc5_:AssetLoaderStruct = this.var_77.assets.loadAssetFromFile(param1,_loc4_,"image/gif");
         if(!_loc5_)
         {
            return;
         }
