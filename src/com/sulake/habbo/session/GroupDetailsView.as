   ...
   public class GroupDetailsView
   {
      ...
      private function showGroupInfoDialog(param1:int) : void
      {
         ...
         if(this.var_156 == null)
         {
            ...
            if(_loc3_ != null)
            {
               this.var_156 = this._sessionManager.windowManager.createWindow("habbo_group_details_window","${group.window.title}",HabboWindowType.const_128,HabboWindowStyle.const_31,0 | 0,new Rectangle(10,10,300,220),null) as IWindowContainer;
               this.var_156.buildFromXML(_loc3_.content as XML) as IWindowContainer;
               _loc4_ = this.var_156.findChildByTag("close");
               if(_loc4_ != null)
               {
               ...
               {
                  _loc7_ = this.var_156.findChildByName(_loc6_);
                  if(_loc7_ != null)
                  {
         ...
         if(this.var_156 != null)
         {
            ...
            this.var_156.center();
            this.var_156.findChildByName("group_name").caption = _loc2_.name;
            this.var_156.findChildByName("group_description").caption = _loc2_.description;
            if(_loc2_.roomId < 0)
            {
               this.var_156.findChildByName("group_room_link").visible = false;
            }
            else
            {
               this.var_156.findChildByName("group_room_link").visible = true;
            }
            this._sessionManager.windowManager.registerLocalizationParameter("group.room.link","room_name",_loc2_.roomName);
            _loc8_ = this.var_156.findChildByName("group_logo") as IBitmapWrapperWindow;
            if(_loc8_ != null)
            {
      ...
      private function onGroupDetailsEvents(param1:Event) : void
      {
         ...
         var _loc3_:HabboGroupDetails = this._sessionManager.getGroupDetails(this.var_2532);
         switch(_loc2_.name)
         {
