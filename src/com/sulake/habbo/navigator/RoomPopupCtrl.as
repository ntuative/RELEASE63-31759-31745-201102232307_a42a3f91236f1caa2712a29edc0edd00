package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2148 = new RoomThumbnailRenderer(param1);
         this.var_1117 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_168 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_168.visible = true;
         Util.hideChildren(this.var_168);
         this.refreshRoomName(this.var_168,this.var_104);
         if(this.var_104.ownerName != "" && this.var_104.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_168,!!this.var_104.event ? this.getEventCtg(this.var_104.categoryId) : this.getRoomCtg(this.var_104.categoryId));
         this.refreshRoomDesc(this.var_168,this.var_104);
         this.refreshExtraCont();
         this.refreshInfo(this.var_168,"trading_allowed",this.var_104.allowTrading);
         this.refreshInfo(this.var_168,"doormode_doorbell",this.var_104.doorMode == 1);
         this.refreshInfo(this.var_168,"doormode_password",this.var_104.doorMode == 2);
         Util.moveChildrenToColumn(this.var_168,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_168.height = Util.getLowestPoint(this.var_168);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_168,this.var_104.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_168.findChildByName("roomowner"));
         if(this.var_104.ownerName != "" && this.var_104.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_168.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1117.refreshTags(_loc1_,this.var_104.tags);
         ...
         this.var_2148.refreshThumbnail(_loc1_,this.var_104.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_250,Tabs.const_242,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
