package com.sulake.habbo.configuration
{
   ...
   public class HabboConfigurationManager extends Component implements IHabboConfigurationManager
   {
      ...
      public function HabboConfigurationManager(param1:IContext, param2:uint = 0, param3:IAssetLibrary = null)
      {
         ...
         this.var_601 = this.getKey("url_prefix");
         if(this.var_601 != null && this.var_601.length > 0)
         {
            this.setKey("client.fatal.error.url",this.var_601 + "/flash_client_error");
         }
         this.init();
      }
      ...
      private function onConfigurationError(param1:Event = null) : void
      {
         ...
         Core.error("Could not load external variables. HTTP status " + _loc3_ + ". Client startup failed!",true,Core.const_1409);
      }
      ...
      private function parseConfiguration(param1:String) : void
      {
         ...
         for each(line in rows)
         {
            if(line.substr(0,1) != "#")
            {
               ...
               if(pair[0].length > 0 && pair[1].length > 0)
               {
                  ...
                  if(key == "configuration.readonly" && value == "HabboCatalogCom_productViewWidget")
                  {
                     readOnly = true;
                  }
         ...
         if(this.getKey("local.environment") == "HabboCatalogCom_productViewWidget")
         {
            this.setKey("url.prefix",this.getKey("url.prefix.local"));
         }
         this.var_601 = this.getKey("url.prefix");
         if(this.var_601.length > 0)
         {
            externalAccessDomain = this.var_601.replace("http://","");
      ...
      public function getKey(param1:String, param2:String = "", param3:Dictionary = null) : String
      {
         ...
         if(_loc4_ != null)
         {
            ...
            {
               _loc4_ = _loc4_.replace("%predefined%",this.var_601);
      ...
      public function setKey(param1:String, param2:String, param3:Boolean = false, param4:Boolean = false) : void
      {
         ...
         if(this.var_1747.indexOf(param1) < 0 || param3)
         {
            this.var_1119[param1] = param2;
         }
         if(param3)
         {
            this.var_1747.push(param1);
         }
      }
