package com.sulake.habbo.quest
{
   ...
   public class QuestCampaign implements IDisposable
   {
      ...
      public function QuestCampaign(param1:QuestController, param2:String)
      {
         ...
         this.var_166 = new QuestHelp(this);
      }
      ...
      private function reset() : void
      {
         ...
         if(this.var_319)
         {
            for each(_loc1_ in this.var_319)
            {
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_1010 = this._window.findChildByName("quest.container") as IWindowContainer;
         this.var_1257 = this._window.findChildByName("currency.container") as IWindowContainer;
         this.var_276 = this._window.findChildByName("quests") as IItemListWindow;
         var _loc1_:* = "icon_" + this.code + "_pixeltype";
         var _loc2_:IBitmapWrapperWindow = this.var_1257.findChildByName("currency.icon") as IBitmapWrapperWindow;
         this._controller.questEngine.setImageFromAsset(_loc2_,_loc1_,this.onPreviewImageReady);
         this.var_1257.findChildByName("currency.amount").caption = "${quest_" + this.var_1521 + ".currency}";
         var _loc3_:String = "quest_" + this.code;
         var _loc4_:* = _loc3_ + ".title";
         var _loc5_:* = _loc3_ + ".currency";
         this.setText("currency.amount",_loc5_);
         this.var_1011 = this._window.findChildByName("help.button") as IWindowContainer;
         if(this.var_1011)
         {
            this.var_1011.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,this.helpButtonEventProc);
            this.var_1011.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_OVER,this.helpButtonEventProc);
            this.var_1011.addEventListener(WindowMouseEvent.const_25,this.helpButtonEventProc);
         }
      }
      ...
      private function openHelp() : void
      {
         ...
         if(this.var_452)
         {
            this.var_452.selected = false;
         }
         if(this.var_166)
         {
            this.var_1010.addChild(this.var_166.view);
         }
      }
      
      public function updatePurse() : void
      {
         var _loc1_:int = this._controller.questEngine.getActivityPointsForType(this.var_1518);
      ...
      private function onPreviewImageReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc2_ != null)
         {
            _loc3_ = this.var_1257.findChildByName("currency.icon") as IBitmapWrapperWindow;
      ...
      public function openCampaign(param1:Array) : void
      {
         ...
         {
            this.var_319.push(new Quest(_loc2_,this));
         }
         _loc3_ = null;
         for each(_loc4_ in this.var_319)
         {
            this.var_276.addListItem(_loc4_.view);
            if(!_loc3_ && (_loc4_.status == QuestStatusEnum.const_46 || _loc4_.status == QuestStatusEnum.const_543))
            {
               _loc3_ = _loc4_;
            }
         }
         ...
         else if(this.var_319.length > 0)
         {
            this.activateListItem(this.var_319[0]);
         }
      }
      ...
      public function activateListItem(param1:Quest) : void
      {
         ...
         this.var_452 = param1;
         this.var_452.selected = true;
         this.showQuestData(this.var_452);
      }
      ...
      private function showQuestData(param1:Quest) : void
      {
         ...
         this.var_1010.addChild(this.var_275.view);
         this.var_275.init();
      }
      ...
      {
         this.modifyQuestData(param1);
         this.var_661.push(new QuestCompleted(this,param1));
      }
      ...
      public function closeQuestCompleted(param1:QuestCompleted) : void
      {
         ...
         var _loc3_:* = null;
         var _loc4_:int = 0;
         for each(_loc3_ in this.var_661)
         {
            if(_loc3_ == param1)
            {
               _loc2_ = _loc3_;
         ...
         {
            _loc2_.dispose();
            _loc4_ = this.var_661.indexOf(_loc2_);
            if(_loc4_ >= 0)
            {
               this.var_661.splice(_loc4_,1);
            }
         }
      }
