package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      public static const const_365:String = "success";
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String, param4:Boolean = true)
      {
         ...
         {
            this.var_1352.init();
            this.var_1352.addEventListener(HabboWeb.const_233,this.requestClientURL);
         }
         else
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         else
         {
            try
            {
               this.var_1681 = /\"sso.ticket\" : \"(.*?)\"/.exec(data)[1];
               this.var_2323 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               facebookData = /\"facebook.user\" : \"(.*?)\"/.exec(data);
               if(facebookData)
               {
                  this.var_2324 = Boolean(facebookData[1]);
               }
               if(this.var_1681.length > 0)
               {
