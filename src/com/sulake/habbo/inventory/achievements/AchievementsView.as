package com.sulake.habbo.inventory.achievements
{
   ...
   public class AchievementsView implements IInventoryView
   {
      ...
      public function AchievementsView(param1:AchievementsModel, param2:IHabboWindowManager, param3:IAssetLibrary, param4:IHabboLocalizationManager, param5:ISessionDataManager)
      {
         ...
         this.var_58 = IItemListWindow(this._view.getChildByName("achievements_itemlist"));
         this.var_393 = new Timer(1000,1);
         this.var_393.addEventListener(TimerEvent.TIMER,this.doBadgeRefresh);
         this.var_404.events.addEventListener(BadgeImageReadyEvent.const_147,this.onBadgeImageReady);
      }
      ...
      public function dispose() : void
      {
         if(!this._disposed)
         {
            ...
            if(this.var_393)
            {
               this.var_393.removeEventListener(TimerEvent.TIMER,this.doBadgeRefresh);
               this.var_393.reset();
               this.var_393 = null;
            }
            this._disposed = true;
      ...
      {
         Logger.log("Got badge: " + param1.badgeId + ", " + this.var_393.running);
         this.var_1050[param1.badgeId] = param1.badgeImage;
         if(!this.var_393.running)
         {
            this.var_393.start();
         }
      }
      ...
      private function refreshBadgeImage(param1:IWindowContainer, param2:AchievementData) : void
      {
         ...
         var _loc4_:BitmapData = this.var_1050[param2.badgeId];
         if(_loc4_ == null)
         {
            _loc4_ = this.var_404.getBadgeImage(param2.badgeId);
            this.var_1050[param2.badgeId] = _loc4_;
         }
