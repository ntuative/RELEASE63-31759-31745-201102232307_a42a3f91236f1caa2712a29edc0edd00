package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1547:int = 4;
      ...
      {
         return this.var_431.length + this.var_270.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_270.length < const_1547 && this.var_431.length > 0)
            {
               _loc1_ = this.var_431.shift();
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_431.indexOf(loader);
            if(index > -1)
            {
               this.var_431.splice(index,1);
            }
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_431)
            {
               if(param2)
               {
